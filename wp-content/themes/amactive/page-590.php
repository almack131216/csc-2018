                        ...
                        foreach($result as $wp_formmaker_submits){
                            ...
                            if($result_migrated) {
                                ...
                                foreach($result_migrated as $wp_migrated_posts){
                                    $id_post = $wp_migrated_posts->id_after;

                                    $result = $wpdb->delete( 'wp_posts', array( 'ID' => $id_post ) );
                                    if($result) echo '<span class="sql_success">DELETED > wp_posts > WHERE ID='.$id_post.'</span>';
                                    if($wpdb->last_error) echo '<span class="sql_error">DELETE FAILED: '.$wpdb->last_error.'</span>';

                                    $result = $wpdb->delete( 'wp_posts', array( 'post_parent' => $id_post ) );
                                    if($result) echo '<span class="sql_success">DELETED > wp_posts > WHERE post_parent='.$id_post.'</span>';
                                    if($wpdb->last_error) echo '<span class="sql_error">DELETE FAILED: '.$wpdb->last_error.'</span>';

                                    $result = $wpdb->delete( 'amactive_migrated', array( 'id_after' => $id_post ) );
                                    if($result) echo '<span class="sql_success">DELETED > amactive_migrated > WHERE id_after='.$id_post.'</span>';
                                    if($wpdb->last_error) echo '<span class="sql_error">DELETE FAILED: '.$wpdb->last_error.'</span>';

                                    $result = $wpdb->delete( 'wp_postmeta', array( 'post_id' => $id_post ) );
                                    if($result) echo '<span class="sql_success">DELETED > wp_postmeta > WHERE post_id='.$id_post.'</span>';
                                    if($wpdb->last_error) echo '<span class="sql_error">DELETE FAILED: '.$wpdb->last_error.'</span>';                                    
            ...
            if(!$isDeleting) {
                ...
                foreach($result as $wp_formmaker_submits){
                    ...
                    if(@is_array(getimagesize($filepath_before))){
                        ...
                        $isImage = false;
                        echo '<span class="sql_error">is NOT an image</span>';
                    }
                    ...
                    if (file_exists($filepath_before) && $isImage) {
                        ...
                        $result = $wpdb->insert('wp_posts', $args);
                        ...
                        echo '<br><strong>STEP 1.2: UPDATE post guid & post_name</strong>';
                        $result = $wpdb->update(
                            'wp_posts',
                            array(
                        ...
                        $result = $wpdb->insert('wp_posts', $args_img);
                        if($result){
                        ...
                        $result = $wpdb->insert('wp_term_relationships', array(
                            'object_id' => $post_id,
                            'term_taxonomy_id' => DV_category_IsForSale_id
                        ));
                        $result2 = $wpdb->insert('wp_term_relationships', array(
                            'object_id' => $post_id,
                        ...
                        // echo 'Q: '.$query;
                        // $wpdb->insert($query);
                        $result = $wpdb->insert('amactive_migrated', $args_migrated);
                        if($result){
                        ...
                        echo '<span class="sql_error">!!! CANNOT FIND IMAGE for item#<a href="http://www.classicandsportscar.ltd.uk/Triumph-GT6-LeMans/classic-cars-sold/'.$item_id.'">'.$item_id.'</a>!!!</span>';
                    }
                }
